        ...
        >
            <Form :model="addForm" :rules="rules" ref="ruleForm" :label-width="95">
                ...
                <FormItem prop="courseName" label="课程名称">
                    <Input v-model="addForm.courseName" placeholder="请输入专业名称" />
                </FormItem>
    ...
    methods: {
        ...
        addHandler(name) {
            this.$refs[name].validate(valid => {
                if (valid) {
                    if (this.isUpdate) {
                        this.$store.dispatch('COURSE_UPDATE', this.addForm)
                            .then(res => {
                                if (res.code == 201) {
                        ...
                        this.$store.dispatch('COURSE_ADD', this.addForm)
                            .then(res => {
                                if (res.code == 201) {
        ...
        delete(id) {
            this.$Modal.confirm({
                ...
                content: '<p>是否删除该课程</p>',
                onOk: () => {
                    this.$store.dispatch('RESOURCE_LIST', `courseId=${id}`)
                        .then(res => {
                            if (res.code == 200) {
                                if (res.data.items.length == 0) {
                                    this.$store.dispatch('COURSE_DELETE', id)
                                        .then(res2 => {
                                            if (res2.code == 201) {
                                    ...
                                    this.$Message.warning('该课程存在资源，无法删除');
                                }
                            } else {
                                this.$Message.error(res.message);
                            }
        ...
        delBatch() {
            ...
            this.$Modal.confirm({
                title: '批量删除',
                content: '<p>是否批量删除选中的课程</p>',
                onOk: () => {
                    this.$store.dispatch('COURSE_DELBATCH', { data: this.selection })
                        .then(res => {
                            if (res.code == 201) {
